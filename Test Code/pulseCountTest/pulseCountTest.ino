...
void setFilter(uint16_t value) { // sets the filtering in # of clock cycles, for debouncing
  if(value==0) {
    pcnt_filter_disable(PCNT_UNIT_0);  
  } else if(value < 1024){ // max filter value is 10 bit so 1023
    pcnt_set_filter_value(PCNT_UNIT_0, value);
...
void loop(){
  Serial.println("Number of falling edges counted: " +String(getCounterValue()));
  delay(1000);
}
